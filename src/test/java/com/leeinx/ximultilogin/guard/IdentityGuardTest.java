...
public class IdentityGuardTest {
    ...
    public void testIdentityGuardBasic() {
        ...
        try {
            ...
            boolean firstVerify = identityGuard.verifyIdentity("testuser", uuid1);
            ...
            // 测试验证相同身份
            boolean secondVerify = identityGuard.verifyIdentity("testuser", uuid1);
            assertTrue("Second verification with same UUID should succeed", secondVerify);

            // 测试验证不同身份
            UUID uuid2 = UUID.randomUUID();
            boolean thirdVerify = identityGuard.verifyIdentity("testuser", uuid2);
    ...
    public void testIdentityGuardNullValues() {
        ...
        try {
            ...
            // 测试空名称
            boolean nullNameVerify = identityGuard.verifyIdentity(null, UUID.randomUUID());
            assertFalse("Verification with null name should fail", nullNameVerify);

            // 测试空 UUID
            boolean nullUuidVerify = identityGuard.verifyIdentity("testuser", null);
            assertFalse("Verification with null UUID should fail", nullUuidVerify);

            // 测试两者都空
            boolean nullBothVerify = identityGuard.verifyIdentity(null, null);
